    ...
    try:
        ...
        with open(file_path, 'rb') as file_obj:
            ...
            try:
                ...
                # If dill fails due to NumPy compatibility, try pickle with fix
                logging.warning(f"Dill failed, trying pickle: {str(dill_err)}")
                file_obj.seek(0)  # Reset file pointer

                import pickle
                obj = pickle.load(file_obj)
                logging.info(f"Object loaded successfully with pickle from {file_path}")
                return obj
